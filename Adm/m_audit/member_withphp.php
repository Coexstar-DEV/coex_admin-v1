  ...
  if(isset($_GET['to']))
  {
        ...
        if($_GET['from'] && $_GET['to'] ){
            ...
            if ( $_GET['m_id'] ){
                ...
                INNER JOIN $member AS m ON m.m_id = t.t_id INNER JOIN $table_setup AS c ON c.c_no = t.t_division WHERE m_id = '$m_id' and c_coin = 'PHP' AND t.t_signdate BETWEEN unix_timestamp('$from') AND unix_timestamp('$to') ORDER BY t.t_signdate DESC  LIMIT 800 "; 
                ...
                $emp_status = strip_tags($_GET['emp_status']);
                $query_pdo = "SELECT m.m_id, m.m_name, m.m_empstatus, m.m_position, m.m_empsalary, m.m_level, FROM_UNIXTIME(t.t_signdate+8*3600) AS t_date, t.t_fees, t.t_delete, t.t_pending,  c.c_coin, t.t_ordermost, t.t_check, TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) as m_age FROM $table_withdraw AS t 
                INNER JOIN $member AS m ON m.m_id = t.t_id INNER JOIN $table_setup AS c ON c.c_no = t.t_division WHERE c_coin = 'PHP' AND t.t_signdate BETWEEN unix_timestamp('$from') AND unix_timestamp('$to') and m.m_empstatus = '$emp_status' and  m.m_empsalary = '$emp_range' and  (TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) BETWEEN '$agerange1' and '$agerange2') ORDER BY t.t_signdate DESC  LIMIT 800 "; 
            }
            else if ($_GET['emp_status'] && $_GET['emp_range']){
                $emp_range = strip_tags($_GET['emp_range']);
                $emp_status = strip_tags($_GET['emp_status']);
                $query_pdo = "SELECT m.m_id, m.m_name, m.m_empstatus, m.m_position, m.m_empsalary, m.m_level, FROM_UNIXTIME(t.t_signdate+8*3600) AS t_date, t.t_fees, t.t_delete, t.t_pending,  c.c_coin, t.t_ordermost, t.t_check, TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) as m_age FROM $table_withdraw AS t 
                INNER JOIN $member AS m ON m.m_id = t.t_id INNER JOIN $table_setup AS c ON c.c_no = t.t_division WHERE c_coin = 'PHP' AND t.t_signdate BETWEEN unix_timestamp('$from') AND unix_timestamp('$to') and m.m_empstatus = '$emp_status' and  m.m_empsalary = '$emp_range' aORDER BY t.t_signdate DESC  LIMIT 800 "; 
            }
            else if ($_GET['emp_status'] && $_GET['m_age']){
                $emp_status = strip_tags($_GET['emp_status']);
                $query_pdo = "SELECT m.m_id, m.m_name, m.m_empstatus, m.m_position, m.m_empsalary, m.m_level, FROM_UNIXTIME(t.t_signdate+8*3600) AS t_date, t.t_fees, t.t_delete, t.t_pending,  c.c_coin, t.t_ordermost, t.t_check, TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) as m_age FROM $table_withdraw AS t 
                INNER JOIN $member AS m ON m.m_id = t.t_id INNER JOIN $table_setup AS c ON c.c_no = t.t_division WHEREc_coin = 'PHP' AND t.t_signdate BETWEEN unix_timestamp('$from') AND unix_timestamp('$to') and m.m_empstatus = '$emp_status'  and  (TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) BETWEEN '$agerange1' and '$agerange2') ORDER BY t.t_signdate DESC  LIMIT 800 "; 
            }
                ...
                $emp_status = strip_tags($_GET['emp_status']);
                $query_pdo = "SELECT m.m_id, m.m_name, m.m_empstatus, m.m_position, m.m_empsalary, m.m_level, FROM_UNIXTIME(t.t_signdate+8*3600) AS t_date, t.t_fees, t.t_delete, t.t_pending,  c.c_coin, t.t_ordermost, t.t_check, TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) as m_age FROM $table_withdraw AS t 
                INNER JOIN $member AS m ON m.m_id = t.t_id INNER JOIN $table_setup AS c ON c.c_no = t.t_division WHERE c_coin = 'PHP' AND t.t_signdate BETWEEN unix_timestamp('$from') AND unix_timestamp('$to') and m.m_empstatus = '$emp_status' ORDER BY t.t_signdate DESC   LIMIT 800"; 
            }
            else if ($_GET['emp_range']){
                $emp_range = strip_tags($_GET['emp_range']);
                $query_pdo = "SELECT m.m_id, m.m_name, m.m_empstatus, m.m_position, m.m_empsalary, m.m_level, FROM_UNIXTIME(t.t_signdate+8*3600) AS t_date, t.t_fees, t.t_delete, t.t_pending,  c.c_coin, t.t_ordermost, t.t_check, TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) as m_age FROM $table_withdraw AS t 
                INNER JOIN $member AS m ON m.m_id = t.t_id INNER JOIN $table_setup AS c ON c.c_no = t.t_division WHERE c_coin = 'PHP' AND t.t_signdate BETWEEN unix_timestamp('$from') AND unix_timestamp('$to') and  m.m_empsalary = '$emp_range' ORDER BY t.t_signdate DESC   LIMIT 800"; 
            }
            else if ($_GET['m_age']){
                $query_pdo = "SELECT m.m_id, m.m_name, m.m_empstatus, m.m_position, m.m_empsalary, m.m_level, FROM_UNIXTIME(t.t_signdate+8*3600) AS t_date, t.t_fees, t.t_delete, t.t_pending,  c.c_coin, t.t_ordermost, t.t_check, TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) as m_age FROM $table_withdraw AS t 
                INNER JOIN $member AS m ON m.m_id = t.t_id INNER JOIN $table_setup AS c ON c.c_no = t.t_division WHERE c_coin = 'PHP' AND t.t_signdate BETWEEN unix_timestamp('$from') AND unix_timestamp('$to')  and  (TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) BETWEEN '$agerange1' and '$agerange2') ORDER BY t.t_signdate DESC   LIMIT 800"; 
            }
            else{
                $query_pdo = "SELECT m.m_id, m.m_name, m.m_empstatus, m.m_position, m.m_empsalary, m.m_level, FROM_UNIXTIME(t.t_signdate+8*3600) AS t_date, t.t_fees, t.t_delete, t.t_pending,  c.c_coin, t.t_ordermost, t.t_check, TIMESTAMPDIFF(YEAR, m_birtday, CURDATE()) as m_age FROM $table_withdraw AS t 
                INNER JOIN $member AS m ON m.m_id = t.t_id INNER JOIN $table_setup AS c ON c.c_no = t.t_division WHERE c_coin = 'PHP' AND t.t_signdate BETWEEN unix_timestamp('$from') AND unix_timestamp('$to') ORDER BY t.t_signdate DESC  LIMIT 800";  
            }
        }
...
// $mpdf->WriteHTML($html);
    ...
    $m_module = "Withdrawal PHP Report";
    $m_type = "Download";
    $m_signdate = time(); 


    $m_modified =  "Generated Withdrawal PHP Report";
