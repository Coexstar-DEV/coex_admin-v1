...
if ($t_check == "") {
	if ($krw == "") {
		if ($t_division != "") {
			...
			$query_pdo = "SELECT t_division,t_no,t_check,t_userno,t_id,t_email,t_ordermost,t_orderkrw,t_depositmost,t_depositkrw,t_fees,t_signdate,t_name,t_acount,t_bankname,t_ip FROM $table_withdraw where t_signdate > ? and t_signdate < ? ";
		}
		$query_pdo .= " and $keyfield LIKE '%$key%' ORDER BY t_signdate DESC";

		$Board_Title = "통합출금내역";
	} else {
		$query_pdo = "SELECT t_division,t_no,t_check,t_userno,t_id,t_email,t_ordermost,t_orderkrw,t_depositmost,t_depositkrw,t_fees,t_signdate,t_name,t_acount,t_bankname,t_ip  FROM $table_withdraw where t_division='1' and t_signdate > ? and t_signdate < ?";
	...
	$query_pdo = "SELECT t_division,t_no,t_check,t_userno,t_id,t_email,t_ordermost,t_orderkrw,t_depositmost,t_depositkrw,t_fees,t_signdate,t_name,t_acount,t_bankname,t_ip FROM $table_withdraw where t_signdate > ? and t_signdate < ? and t_division='1' and t_check='0'";
...
<body>
	<table width="100%" border='1' cellspacing='0' cellpadding='0'>
		<tr align="center">
			...
			<td width="50" height="30"><?=M_DONE?></td>
			<td width="300" height="30"><?=M_ID?>(<?=M_NO?>)</td>
			<td width="150" height="30"><?=M_NAME?></td>
			<td width="150" height="30"><?=M_BANK?></td>
			<td width="150" height="30"><?=M_ACCOUNT?></td>
			<td width="180" height="30"><?=M_ORDER_COST?></td>
			<td width="180" height="30"><?=M_ORDER.M_KRW?></td>
			<td width="180" height="30"><?=M_CLOSE_COST?></td>
			<td width="180" height="30"><?=M_DONE.M_KRW?></td>
			<td width="120" height="30">지급</td>
		...
		<tr align="center" bgcolor="<?= $kk_bgcolor ?>">
			<td height="40"><?= $t_no ?></td>
			<td height="40" align="center"><?= $t_division ?></td>
			<td height="40" align="center"><?= $t_check ?></td>
			<td height="40"><?= $t_id ?>(<?= $t_userno ?>)</td>
			<td height="40"><?= $t_name ?></td>
			<td width="30" height="40"><?= $t_bankname ?></td>
			<td width="30" height="40" style="text-align:center"><?= $t_acount ?></td>
			<td height="40"><B><?= number_format($t_ordermost) ?></B> </td>
			<td height="40" align="center"><?= number_format($t_orderkrw) ?></td>
			<td height="40" align="center"><?= number_format($t_depositmost) ?></td>
			<td height="40"><?= $t_depositkrw ?></td>
			<td height="40"><?= $t_signdate ?></td>
			<td height="40"><? if ($t_check == "X") { ?><a onclick="go_highpass(<?= $t_no ?>);" style="cursor:pointer">지급<? } else { ?>지급완료<? } ?></td>
			<td height="40"><?= $t_ip ?></td>
		</tr>
		<?
			$article_num--;
			$ii++;
