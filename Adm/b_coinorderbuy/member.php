...
$table_orderbuy = $div == "1" ? $table_orderbuy : $table_orderbuy;
...
$IsNext = is_empty($_REQUEST["IsNext"]) ? 0 : $_REQUEST["IsNext"];
...
####코인 확인##########
$query_pdo = "SELECT b_no,b_state,b_ordermost,b_closecost,b_closefees,b_orderprice,b_pricetotal,b_closeprice,b_closetotal,b_userno,b_id,b_signdate,b_ip,b_delete,b_closedate,b_orderfees,b_pay,b_market  FROM $table_orderbuy where b_div =? ";
...
$total_record_pdo = pdo_excute_count("count", $query_pdo, [$b_div_div]);
$num_per_page = 10;
$page_per_block = 10;
...
<a href=' ../b_coinorderbuy/member.php?b_statee=com&b_div_div=<?= $b_div_div ?>''><?= $c_coinname ?><?=M_BUY.M_DONE.M_HIS?> </a> <span style="border-right:1px solid #88b7da; margin-right:20px;"> </span>
		<a href='../b_coinordersell/member.php?b_statee=com&b_div_div=<?= $b_div_div ?>''><?= $c_coinname ?><?=M_SELL.M_DONE.M_HIS?></a>
				...
				<table align="center" width="1400" border="0" cellspacing="0" cellpadding="0" class="left_margin30">
					...
					<tr><td>
							<table align="center" width="100%" border=0 cellpadding=0 cellspacing=0>
								<tr>
									<td class=' td14' align="center"><? if ($b_statee != "com") { ?><?= $c_coinname ?><?=M_BUY.M_ORDER.M_HIS?><? } else { ?><?= $c_coinname ?><?=M_BUY.M_DONE.M_HIS?><? } ?></td>
			<td align="right">
				<form name=dform action="./member_dis_excel.php" method=post target="_blank">
					...
					<input type="hidden" name="b_statee" value="<?= $b_statee ?>">
			...
			<form name="form" method="post">
				...
				<tr>
					<td>
						<table width="1200" align="center" border="0" cellspacing="0" cellpadding="4">
							<tr>

								<td height="20" align="left">
									<select name="pay_type">
										<option value="999">COIN</option>
										<?
						...
						<table width="1200" border='0' align="center" cellspacing='0' cellpadding='0' class="sub_title">
							...
							<tr align="center" bgcolor='#ffffff' class="list_title">
								...
								<td width="100" height="30"><?=M_AMOUNT?></td>
							...
							while ($row = $stmt->fetch()) {
								...
								$b_delete = $row[13];
								$b_closedate = $row[14];
								$b_orderfees = numberformat($row[15], "money3", $digit);
								$b_pay = $row[16];
								$b_market = $row[17];
