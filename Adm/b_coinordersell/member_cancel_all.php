...
try {
	...
	while ($row = $stmt->fetch()) {
		...
		if (is_empty($sellInfo->userInfo->id)) {
			fatal_log(__FILE__."(".__LINE__."):"."FAIL_FIND_USER[".$userid."/".$sellInfo->userInfo->id."]");
		...
		$stmt1 = pdo_excute("sell_cancel", $query, $pdo_in);
		...
		if ($result == 5) {
			...
			fatal_log("====>SELL($b_no) Failed refund:".$result);
