...
$m_type = "Cancel";
$m_modified = "Canceled sell of" . " " . $b_id;
try {

    $sellInfo = SellInfo::makeFromNo($b_no);
    //err_log(var_export($sellInfo, true));

    if (is_empty($sellInfo->userInfo->id)) {
        fatal_log(__FILE__."(".__LINE__."):"."FAIL_FIND_USER[".$b_no."]");
    ...
    $query .= " FROM dual";
    $pdo_in = [$sellInfo->userInfo->no, $sellInfo->orderCoin->type, $sellInfo->payCoin->type, 0, 0, $userip, $signdate, $sellInfo->no];

    $stmt = pdo_excute("sell_cancel", $query, $pdo_in);
    $row = $stmt->fetch();
    ...
    if ($result == 5) {
        ...
        fatal_log("INVALID_TRADE_INFO :result:$result");
        throw new Exception("INVALID_TRADE_INFO", __LINE__);
    }
	err_log("====>SELL($b_no) Refund done:".$result);
	$updated = true;
    
} catch(Exception $e) {
    $s = $e->getMessage() . ' (code:' . $e->getCode() . ')';
    fatal_log("====> SELL($b_no) Failed refund:".$s);
