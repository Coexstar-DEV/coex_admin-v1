...
if (isset($_POST["s_userno"])) {
    $s_userno = sqlfilter($_POST["s_userno"]);
} else {
    $s_userno = "";
}

if (isset($_POST["s_tno"])) {
    $s_tno = sqlfilter($_POST["s_tno"]);
} else {
    $s_tno = "";
}

if (isset($_POST["s_mno"])) {
    $s_mno = sqlfilter($_POST["s_mno"]);
} else {
    $s_mno = "";
}
...
$enddate = mktime(date("H"),date("i"),date("s"),date("m"),date("d")+100,date("Y"));
...
try {
	...
	$withdraw->columns["t_fees"] = "0";
    // $withdraw->transmit($userInfo, $t_ordermost, "0", "AF1");
    $withdraw->transmit($userInfo, $t_ordermost, "0", "AF1", "1", $signdate, $enddate, $s_mno);
...
// $query6 = "UPDATE $stake SET m_status = 1, m_startdate = '$signdate', m_enddate = '$enddate' WHERE m_userno = '$t_userno' AND m_tno = '$t_no'";
...
echo ("<meta http-equiv='Refresh' content='0; URL=member.php?keyfield=$keyfield&key=$encoded_key&page=$page&krw=$krw'>");
?>